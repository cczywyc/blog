...
{% mermaid %}
flowchart TD
    W4["第四周 RAG<br/>造出「向量检索器」retriever"]
    W5["第五周记忆<br/>长期事实的「语义召回」就是再调一次它"]
    W4 -->|直接复用| W5
{% endmermaid %}
...
flowchart TD
    A["agent.py"]
    A -->|"轮前调用 · 读（挑选）"| AC["assemble_context()"]
    A -->|"轮后调用 · 写（积累）"| UP["update_from_turn()"]
    AC --> MM
    UP --> MM
    subgraph MM["MemoryManager"]
        ST["短期记忆<br/>最近 K 轮完整对话<br/>（轮次 + 字符双闸门）"]
        LT["长期记忆<br/>偏好：key-value<br/>事实：list + 向量库<br/>主题：counter"]
    end
    ST -->|"evict：超出 K 轮"| SU["摘要器"]
    SU -->|"高保真：带引用的结论，晋升"| LT
    SU -->|"低保真：压成一句话（≤80 字）"| SM["累积摘要"]
{% endmermaid %}
...
{% mermaid %}
flowchart LR
    A["最该丢<br/>旧检索 chunk<br/>（可重新检索）"] --> B["旧网页正文"] --> C["旧工具记录"] --> D["旧模型回答"] --> E["最不该丢<br/>最近 K 轮<br/>（不可重建，不丢）"]
{% endmermaid %}
...
flowchart TD
    O["超出 K 轮的旧轮次"]
    O -->|"高保真通道"| H["带 doc#section 引用的结论"]
    O -->|"低保真通道"| L["剩余的过程性对话"]
    H -->|"晋升（零信息损失）"| F["长期「已确认事实」<br/>带来源、可语义召回"]
    L -->|"调一次模型"| S["压成 ≤80 字摘要<br/>追加进累积摘要"]
    S --> D["旧轮次真正删除"]
{% endmermaid %}
...
flowchart TD
    Q{"装配后超预算？"}
    Q -->|否| OK["直接发给模型"]
    Q -->|是，第一刀| S5["裁段 5：从最旧轮开始<br/>assistant 回答截到 200 字<br/>（user 输入保全文——更短，且不可重生）"]
    S5 -->|仍超，第二刀| S34["裁段 3/4：摘要保尾部削头部<br/>事实按整条削尾，不切坏半条"]
    S34 --> NV["段 1/2/6 永不裁<br/>（系统约束、偏好、当前问题动谁都不行）"]
    NV --> OK
{% endmermaid %}
...
flowchart TD
    U["用户输入"]
    U --> AC["轮前：assemble_context()<br/>读 — 偏好全量、摘要、事实语义召回、最近 K 轮"]
    AC --> LOOP["Agent Loop<br/>（模型决策 + 工具调用，循环若干次）"]
    LOOP --> UP["轮后：update_from_turn()<br/>写 — 主题计数 → 偏好抽取 → 事实晋升<br/>→ 本轮入短期 → 必要时摘要 + evict"]
    UP --> SV["save()<br/>6 个 json + 2 个向量库全量落盘"]
{% endmermaid %}
...
图里最后的 `save()` 节点也值得一提：每轮结束全量落盘 6 个 json 加 2 个向量库，跨会话还要手动加载快照。这是整个实现里最琐碎、最容易出错的部分——不是设计难，是纯粹的脏活。这份体感对下周很重要，后面说。
